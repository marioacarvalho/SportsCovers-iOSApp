...
- (void)renderLayerWithView:(UIView*)superview
{
    ...
    for (UIView *subview in view.subviews) {
        ...
        if ([subview isKindOfClass:[CDCountriesSelectorView class]]) {
            [view addSubview:subview];
        }
    }
    //get the visible rect
    CGRect visibleRect = [view convertRect:self.frame toView:self];
    ...
    [self toggleBlurViewsInView:view hidden:YES alpha:alpha];
    ...
    [self toggleBlurViewsInView:view hidden:NO alpha:alpha];
...
{    
    ...
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_USEC));
